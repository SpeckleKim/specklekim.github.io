...
도구 사용(함수 호출이라고도 함)은 언어 모델이 추론 중에 외부 함수와 API를 호출할 수 있게 합니다. 텍스트만 생성하는 대신 LLM은 이제 도구를 호출할 수 있습니다. 데이터베이스 검색, 코드 실행, 실시간 데이터 가져오기, 장치 제어로 모델을 외부 시스템과 상호 작용할 수 있는 에이전트로 만듭니다. 도구 사용은 실질적인 AI 시스템 구축의 기초입니다.
...
OpenAI의 GPT 모델은 기본적으로 함수_호출을 지원합니다:
...
도구 사용은 LLM을 텍스트 생성기에서 실제 시스템과 상호 작용할 수 있는 에이전트로 변환합니다. 효과적인 도구 사용은 명확한 API 설계, 견고한 오류 처리, 신중한 보안 경계가 필요합니다. 도구 생태계가 성숙해지면서 에이전트 시스템은 점점 더 강력하고 실질적이 됩니다.
